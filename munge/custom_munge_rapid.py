...
reverse_hg38_dbsnp = "dbsnp/sorted_1kg_matched_reversed_hg38_snp150.txt.gz"
reverse_hg19_dbsnp = "dbsnp/sorted_1kg_matched_reversed_hg19_snp150.txt.gz"
...
def main():
    ...
    for genome in config["genome_build"]:
        if genome == "hg38":
            config["rsid_to_pos"] = reverse_hg38_dbsnp
        elif genome == "hg19":
            config["rsid_to_pos"] = reverse_hg19_dbsnp
        else:
...
def write_tmp_file(study, trait, in_file, config):
    ...
    with open(config["tmp_file"], "w") as w:
		...
		pos_tuple = get_pos_from_rsid(config["rsid_to_pos"], raw_rs)
                if pos_tuple != "":
                    chrom, snp_pos = pos_tuple[0], pos_tuple[1] 
                else:
                    continue
                ...
                raw_chrom = data[int(study["chr_index"]) - 1]
                raw_snp_pos = data[int(study["snp_pos_index"]) - 1]

                if study["chr_index"] == study["snp_pos_index"]:
                    raw_chrom = chrom.split(study["snp_split_char"])[0]
                    raw_snp_pos = snp_pos.split(study["snp_split_char"])[1]

                # Get rid of the line if there's no chrom or pos
                if raw_chrom.strip() == "" or raw_snp_pos.strip() == "":
                    continue

                # Get rid of the line if chromosome isn't valid
                if raw_chrom not in valid_chroms:
                    continue
                raw_chrom = raw_chrom.replace('chr', '')
                try:
                    raw_chrom = int(raw_chrom)
                except:
                    continue

                raw_snp_pos = int(float(raw_snp_pos)) 

                # Skip the line if it can't be mapped to a known rsid
		rsid = get_rsid_from_pos(config["pos_to_rsid"][study["source_build"]], raw_chrom, raw_snp_pos)
                if rsid == "":
                    continue
            ...
            out_data.extend([rsid, chrom, snp_pos])
...
def get_source_build(study, config, iters = 100000):
    ...
    with open_fct(in_file) as f:
        ...
        for i in xrange(iters): 
            ...
            get_rsid_from_pos(config["pos_to_rsid"][build])
            if rsid != "":
                scores[build] += 1
...
def load_pos_to_rsid_all(config):
    pos_to_rsid = {}
    pos_to_rsid["hg18"] = hg18_dbsnp
    pos_to_rsid["hg19"] = hg19_dbsnp
    pos_to_rsid["hg38"] = hg38_dbsnp
    return pos_to_rsid
...
def get_pos_from_rsid(infile, rsid):
    data = subprocess.check_output("tabix {0} 1:{1}-{1}".format(infile, rsid), shell=True).strip()
    if data == "":
        return ""

    # This isn't perfect, but we just take the first one for now if there are multiple matches
    data = data.split("\n")[0]
    data = data.strip().split("\t")
    chrom, pos = data[2], data[3]

    return (chrom, pos)

def get_rsid_from_pos(infile, chrom, pos):
    data = subprocess.check_output("tabix {0} {1}:{2}-{2}".format(infile, chrom, pos), shell=True).strip()
    if data == "":
        return ""
